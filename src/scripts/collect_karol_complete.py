#!/usr/bin/env python3
"""
Script completo para coleta de tweets do caso Karol Conká
Combina login e coleta em um único processo para evitar problemas de sessão
"""
...
def parse_arguments():
    ...
    parser.add_argument('--skip-login', action='store_true',
                       help='Pular login se já estiver logado')
...
def login_to_twitter(page):
    """Realiza login no Twitter"""
    print("\n🔐 FAZENDO LOGIN NO TWITTER")
    print("="*50)
...
def main():
    ...
    with sync_playwright() as p:
        ...
        if not args.skip_login:
            ...
            print("⏭️ Pulando login (--skip-login ativado)")
        ...
        try:
            ...
            if "password" in content or "log in" in content or "entrar" in content:
                print("❌ Não logado no X/Twitter. Tente fazer login novamente.")
                browser.close()
                return
            else:
