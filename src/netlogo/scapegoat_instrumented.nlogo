     ...
     set stopsign red
          ...
          [
                ...
                set faccused 1
                    ...
                    set shape "x"
                    set size 2
                    ask src [ log-event src tgt "faccuse" ]
                          ]
                          ask households
                        [
                    set accuseswitch red
...
to-report kind-of [t]  ;; t é a turtle (household)
  let shp [shape] of t
  report ifelse-value (shp = "square")   [ "leader" ]
    [ ifelse-value (shp = "star")        [ "victim" ]
      [ ifelse-value (shp = "triangle")  [ "accuser_failed" ]
        [ ifelse-value (shp = "x")       [ "victim_failed" ]
          [ "neutral" ] ] ] ]
end

to set-kind-labels
  ask households [
    set label kind-of self
  ]
end

to export-node-kinds [filepath]  ;; ex.: "C:/Usuarios/Paes1/NETLOGO/exports/nodes_kind.csv"
  let rows (list (list "Id" "kind" "label"))
  ask households [
    let k kind-of self
    set label k
    set rows lput (list who k label) rows
  ]
  csv:to-file filepath rows
  show (word "Salvo: " filepath " (n=" count households ")")
end
